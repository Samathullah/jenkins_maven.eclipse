...
class SongCollectionTest {
	...
	void testSortSongsByTitle() {
		...
		sc.sortSongsByTitle();
		// Assert that Song Collection is equals to Song Collection Size + 1 : 5
		assertEquals(testSc.get(0).getTitle(), "MONTERO");
		assertEquals(testSc.get(1).getTitle(), "Peaches");
		assertEquals(testSc.get(2).getTitle(), "bad guy");
		assertEquals(testSc.get(3).getTitle(), "good 4 u");
	}
	...
	void testSortSongsBySongLength() {
		...
		// Assert that Song Collection is equals to Song Collection Size + 1 : 5
		assertEquals(testSc.get(0).getSongLength(), 3.59);
		assertEquals(testSc.get(1).getSongLength(), 3.18);
	...
	void testFindSongsById() {
		...
		List<Song> testSc = sc.getSongs();
		// Act
		sc.findSongsById(null);
		// Assert that Song Collection is equals to Song Collection Size + 1 : 5
		assertEquals(testSc.get(0).getId(), "001");
		assertEquals(testSc.get(1).getId(), "002");
	}
	...
	void testFindSongByTitle() {
		...
		sc.findSongByTitle(null);

		assertEquals(testSc.equals(testSc), true);
	}
